<form action="{{ url('/stok/import_ajax') }}" method="POST" id="form-import-stok" enctype="multipart/form-data">
    @csrf
    <div id="modal-master-stok" class="modal-dialog modal-lg" role="document">
    ...
    $("#form-import-stok").validate({
        ...
        submitHandler: function(form) { 
            ...
            $.ajax({
                ...
                processData: false, // Disable processData and contentType to handle file
                contentType: false,
                success: function(response) {
                    if(response.status){ // If success
                        $('#modal-master-stok').modal('hide');
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
                            text: response.message
                        });
                        tableStok.ajax.reload(); // Reload datatable for stok
                    } else { // If error
                        $('.error-text').text('');
